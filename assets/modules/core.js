...
async function adminRefreshUsers() {
  ...
  // Load profiles + signin docs, then merge by uid.
...
async function adminPreviewCharacter(uid, charIndex) {
  ...
  // Snapshot current local state so exit is clean.
  ...
  // Put into a temporary local list so the existing UI can render it.
  const local = [clone];
  setStoredJSON('dndCharacters', local);
...
function disableEditingForAdminPreview(on) {
  ...
  root.querySelectorAll('input, textarea, select, button').forEach(el => {
    // Keep Exit Preview button enabled (outside #pages anyway), and allow tabs/settings navigation.
